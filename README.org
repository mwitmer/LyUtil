...
instrument; eg. (violin . "I") or (bassoon . 2). Any datum that can be
formated meaningfully by Guile's /format/ method can be used.
...
- *part-overrides* /alist/: An alist of paper and layout blocks to use
  in specific parts. The keys in the alist are instrument specifiers
  -- that is, either a symbol that keys to a specific instrument,
  ie. 'violin or 'flute, or a pair such as '(violin . "I") or
  whatnot. Don't use specifiers for combined parts because parts are
  made for each part individually. The value is another alist, which
  can have up to two elements -- one with key 'paper whose value is a
  paper block, and one with key 'layout whose value is a layout
...
\secondaryfont markup command. They are initially defined as a no-op,
...
LilyPond's documentation for \instrumentSwitch specifies the
...
*\noPartBreak* are defined for managing page (not line!) breaks in
parts.
** Cues
...
part's header that no
rmally wouldn't be there, add it to
